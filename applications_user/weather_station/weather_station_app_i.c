...
void ws_preset_init(
    ...
    FURI_LOG_I(
        TAG,
        "ws_preset_init name=%s freq=%lu preset_data=%p size=%zu",
        preset_name,
...
void ws_begin(WeatherStationApp* app, uint8_t* preset_data) {
    furi_assert(app);
    if(!preset_data) {
        FURI_LOG_E(TAG, "ws_begin called with null preset_data");
    } else {
        FURI_LOG_I(TAG, "ws_begin loading preset data %p", (void*)preset_data);
    }
...
uint32_t ws_rx(WeatherStationApp* app, uint32_t frequency) {
    furi_assert(app);
    FURI_LOG_I(TAG, "ws_rx request frequency=%lu current_state=%d", (unsigned long)frequency, app->txrx->txrx_state);
    ...
    FURI_LOG_I(TAG, "ws_rx set_frequency returned=%lu", (unsigned long)value);
    ...
    FURI_LOG_I(TAG, "ws_rx started async RX on device %s", subghz_devices_get_name(app->txrx->radio_device));
