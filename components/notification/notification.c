...
static void notification_reset_notification_layer(
    ...
    } else if(furi_timer_is_running(app->display_timer)) {
...
static bool notification_load_settings(NotificationApp* app) {
    ...
    /* Preserve 0 = Always ON while still clamping unintended low values. */
    if(app->settings.display_off_delay_ms != 0 &&
       app->settings.display_off_delay_ms < 2000) {
        app->settings.display_off_delay_ms = 2000;
    }
    return ok;
}

static bool notification_save_settings(NotificationApp* app, const NotificationSettings* settings) {
    FURI_LOG_I(TAG, "SAVE start: %u bytes", (unsigned)sizeof(NotificationSettings));
    uint32_t t0 = furi_get_tick();
    bool ok = false;

    /* Build header + payload identical to SavedStruct format, then write to
     * filesystem under /int to avoid blocking NVS commit path which can trigger
     * watchdog resets. */
    typedef struct {
        uint8_t magic;
        uint8_t version;
        ...
        uint8_t flags;
        uint32_t timestamp;
    } __attribute__((packed)) FileSavedStructHeader;

    FileSavedStructHeader header = {
    ...
    for(size_t i = 0; i < sizeof(NotificationSettings); i++) checksum += src[i];
    header.checksum = checksum;

    size_t blob_size = sizeof(header) + sizeof(NotificationSettings);
    uint8_t* blob = malloc(blob_size);
    if(blob) {
        memcpy(blob, &header, sizeof(header));
        memcpy(blob + sizeof(header), settings, sizeof(NotificationSettings));

        Storage* storage = furi_record_open(RECORD_STORAGE);
        if(storage) {
            File* file = storage_file_alloc(storage);
            if(file) {
                if(storage_file_open(file, NOTIFICATION_SETTINGS_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
                    size_t written = storage_file_write(file, blob, blob_size);
    ...
    uint32_t elapsed_ms = (uint32_t)(((uint64_t)(t1 - t0) * 1000ULL) / furi_kernel_get_tick_frequency());
    if(ok) {
        FURI_LOG_I(
            TAG,
            "SAVE ok: brightness=%.2f vol=%.2f elapsed=%ums",
            (double)settings->display_brightness,
            (double)settings->speaker_volume,
            (unsigned)elapsed_ms);
    } else {
        FURI_LOG_E(TAG, "SAVE failed elapsed=%ums", (unsigned)elapsed_ms);
    }
    return ok;
}
...
static inline void notification_led_mutex_lock(NotificationApp* app) {
    if(app && app->led_mutex) {
        furi_mutex_acquire(app->led_mutex, FuriWaitForever);
    }
}

static inline void notification_led_mutex_unlock(NotificationApp* app) {
    if(app && app->led_mutex) {
        furi_mutex_release(app->led_mutex);
    }
}
...
static void notification_led_effect_tick(void* context) {
    furi_assert(context);
    NotificationApp* app = context;
    notification_led_mutex_lock(app);
    led_effect_phase++;
    ...
    notification_led_mutex_unlock(app);
}
...
static NotificationApp* notification_app_alloc(void) {
    ...
    app->led_mutex = furi_mutex_alloc(FuriMutexTypeRecursive);

    app->settings.display_brightness = 1.0f;
    app->settings.display_off_delay_ms = 0; /* Always on by default */
...
int32_t notification_srv(void* p) {
    ...
    while(true) {
        ...
        case InternalLayerMessage:
            ...
            break;
        case ApplyLedColorMessage:
            notification_apply_led_color(app);
            break;
        case SaveSettingsMessage:
            notification_save_settings(app, &message.settings);
            break;
        }
...
void notification_message(NotificationApp* app, const NotificationSequence* sequence) {
    notification_message_send(app, NotificationLayerMessage, sequence, NULL, NULL);
}
...
void notification_message_block(NotificationApp* app, const NotificationSequence* sequence) {
    ...
    notification_message_send(app, NotificationLayerMessage, sequence, back_event, NULL);
...
void notification_internal_message(NotificationApp* app, const NotificationSequence* sequence) {
    notification_message_send(app, InternalLayerMessage, sequence, NULL, NULL);
}
...
void notification_internal_message_block(
    ...
    notification_message_send(app, InternalLayerMessage, sequence, back_event, NULL);
    furi_event_flag_wait(
        back_event, NOTIFICATION_EVENT_COMPLETE, FuriFlagWaitAny, FuriWaitForever);
    furi_event_flag_free(back_event);
}

void notification_message_apply_led_color(NotificationApp* app) {
    furi_assert(app);
    notification_message_send(app, ApplyLedColorMessage, &sequence_empty, NULL, NULL);
}

void notification_message_save_settings(NotificationApp* app) {
    furi_assert(app);
    notification_message_send(app, SaveSettingsMessage, &sequence_empty, NULL, &app->settings);
}
