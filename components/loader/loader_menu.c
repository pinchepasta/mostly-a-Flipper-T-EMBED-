...
static void loader_menu_switch_to_settings(void* context, uint32_t index) {
    ...
    if(!app->settings_submenu_built) {
        loader_menu_build_submenu(app, app->loader_menu);
        app->settings_submenu_built = true;
    }
...
static LoaderMenuApp* loader_menu_app_alloc(LoaderMenu* loader_menu) {
    ...
    app->settings_submenu_built = false;
