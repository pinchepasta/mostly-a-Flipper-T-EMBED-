...
void furi_hal_rtc_init_early(void) {
    /*
     * When this firmware is launched as an app from another launcher/firmware,
     * stale boot-mode state can survive in RAM and make the new image behave as
     * if it was booted for an update/DFU flow. Resetting the entire RTC state
     * here guarantees a clean normal boot path for the launched firmware.
     */
