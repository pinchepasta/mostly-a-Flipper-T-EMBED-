...
// INTERCEPT ROUTINES
//
/* ESP32 port: allocated from PSRAM in P_AllocIntercepts(). */
intercept_t*	intercepts = NULL;
intercept_t*	intercept_p;

#include <esp_heap_caps.h>
void P_AllocIntercepts(void)
{
    if (!intercepts)
        intercepts = heap_caps_calloc(MAXINTERCEPTS, sizeof(intercept_t), MALLOC_CAP_SPIRAM);
}
