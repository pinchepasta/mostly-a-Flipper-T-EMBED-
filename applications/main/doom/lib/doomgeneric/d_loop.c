...
/* ESP32 port: ~20 KB. Allocated from PSRAM via D_AllocTicdata() at app start. */
static ticcmd_set_t* ticdata = NULL;

#include <esp_heap_caps.h>
void D_AllocTicdata(void)
{
    if (!ticdata)
        ticdata = heap_caps_calloc(BACKUPTICS, sizeof(ticcmd_set_t), MALLOC_CAP_SPIRAM);
}
