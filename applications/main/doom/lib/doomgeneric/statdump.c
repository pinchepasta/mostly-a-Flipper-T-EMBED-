...
/* ESP32 port: allocated from PSRAM in I_AllocCapturedStats(). */
static wbstartstruct_t* captured_stats = NULL;
static int num_captured_stats = 0;

#include <esp_heap_caps.h>
void I_AllocCapturedStats(void)
{
    if (!captured_stats)
        captured_stats = heap_caps_calloc(MAX_CAPTURES, sizeof(wbstartstruct_t), MALLOC_CAP_SPIRAM);
}
