...
/* ESP32 port: PSRAM is shared with WAD cache, LCD buffers and other app
 * allocations — try to take as much as we can but do not insist on 6 MB. */
...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram)
{
    ...
    while (zonemem == NULL)
    {
        ...
        // Try to allocate the zone memory (ESP32 port: force PSRAM).
