...
/* ESP32 port: keep the master copy in rodata (→ PSRAM via
 * CONFIG_SPIRAM_RODATA=y) and allocate the mutable working copy from
 * PSRAM in I_InitMutableTables(), called from DG_Init().
 * The extern declaration in info.h is `state_t states[]` — when the
 * symbol resolves to a pointer the callers still use `states[i]`
 * unchanged. */
...
/* ESP32 port: runtime-allocated copies of the master tables live in PSRAM so
 * they do not consume DRAM unless Doom is actually running. The allocators
 * are called from DG_Init(). */
#include <string.h>
#include <esp_heap_caps.h>
state_t*    states = NULL;
mobjinfo_t* mobjinfo = NULL;

void I_InitMutableInfoTables(void)
{
    if (!states) {
        states = heap_caps_malloc(sizeof(states_init), MALLOC_CAP_SPIRAM);
        if (states) memcpy(states, states_init, sizeof(states_init));
    }
    if (!mobjinfo) {
        mobjinfo = heap_caps_malloc(sizeof(mobjinfo_init), MALLOC_CAP_SPIRAM);
        if (mobjinfo) memcpy(mobjinfo, mobjinfo_init, sizeof(mobjinfo_init));
    }
}
