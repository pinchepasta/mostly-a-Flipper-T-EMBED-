...
int		viewwindowx;
int		viewwindowy; 
/* ESP32 port: allocated from PSRAM in R_AllocDrawTables(). */
byte**		ylookup = NULL;
int*		columnofs = NULL;

#include <esp_heap_caps.h>
void R_AllocDrawTables(void)
{
    if (!ylookup)   ylookup   = heap_caps_calloc(MAXHEIGHT, sizeof(byte*), MALLOC_CAP_SPIRAM);
    if (!columnofs) columnofs = heap_caps_calloc(MAXWIDTH,  sizeof(int),   MALLOC_CAP_SPIRAM);
}
