...
static void karma_apply_ssid(const char* ssid) {
    ...
    // Clamp channel to valid range (1-11 for most regulatory domains like US/cc=01)
    uint8_t ch = s_channel ? s_channel : 1;
    ap_cfg.ap.channel = (ch > 11) ? 11 : ch;
...
static void evil_portal_start_worker(void* arg) {
    ...
    // Clamp channel to valid range (1-11 for most regulatory domains like US/cc=01)
    uint8_t ch = cfg->channel ? cfg->channel : 1;
    ap_cfg.ap.channel = (ch > 11) ? 11 : ch;
    if(cfg->channel > 11) {
        ESP_LOGW(TAG, "[worker] Channel %u out of valid range, clamped to %u", 
                 cfg->channel, ap_cfg.ap.channel);
    }
    ap_cfg.ap.authmode = WIFI_AUTH_OPEN;
    ap_cfg.ap.max_connection = 4;
    ap_cfg.ap.beacon_interval = 100;
