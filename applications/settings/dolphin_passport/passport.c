...
static void input_callback(InputEvent* input, void* ctx) {
    PassportContext* passport_ctx = ctx;

    if((input->type == InputTypeShort) && (input->key == InputKeyBack)) {
        furi_semaphore_release(passport_ctx->semaphore);
    }
}
...
int32_t passport_app(void* p) {
    ...
    PassportContext* passport_ctx = malloc(sizeof(PassportContext));
    passport_ctx->semaphore = semaphore;
    
    // Copy current name to buffer for reference
    const char* current_name = furi_hal_version_get_name_ptr();
    if(current_name) {
        strncpy(passport_ctx->name_buffer, current_name, NAME_BUFFER_SIZE - 1);
        passport_ctx->name_buffer[NAME_BUFFER_SIZE - 1] = '\0';
    } else {
        passport_ctx->name_buffer[0] = '\0';
    }

    view_port_draw_callback_set(view_port, render_callback, passport_ctx);
    view_port_input_callback_set(view_port, input_callback, passport_ctx);
